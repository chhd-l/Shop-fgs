...
import { passwordByForgot, checkVerifyCode } from '@/api/login';
...
class ResetPassword extends React.Component {
  ...
  componentDidMount() {
    const searchEmail = funcUrl({ name: 'email' });
    const searchVerifyCode = funcUrl({ name: 'verifyCode' });
    if (searchEmail && searchVerifyCode) {
      this.verifyCodeExpired(searchEmail);
      this.setState({
        searchEmail,
        searchVerifyCode
      });
  ...
  verifyCodeExpired = async (searchEmail) => {
    this.setState({
      loading: true
    });
    try {
      const res = await checkVerifyCode({ customerAccount: searchEmail });
      if (!res?.context?.isNotExpireTimeFlag) {
        this.props.history.push('/reset/error');
      }
    } catch (e) {
      this.props.history.push('/reset/error');
    } finally {
  ...
  handleReset = async () => {
    ...
    try {
      ...
      this.props.history.push('/reset/failure');
    } finally {
      this.setState({
        loading: false
      });
  ...
  render() {
    ...
    return (
      <div>
        <div className="flex flex-col items-center mt-8 mb-24 reset-password-wrap">
          ...
          <div className="w-full md:w-96 px-6 md:px-0">
            {errorMsg ? (
              <aside className="rc-alert rc-alert--error mb-8 " role="alert">
