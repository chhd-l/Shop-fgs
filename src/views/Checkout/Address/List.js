...
class AddressList extends React.Component {
  ...
  get panelStatus() {
    ...
    return this.props.paymentStoreNew[tmpKey];
  }
  get curPanelKey() {
  ...
  async queryAddressList({ init = false } = {}) {
    ...
    try {
      ...
      // 查询银行卡列表
      // this.isDeliverAddress && this.props.paymentStoreNew.setDefaultCardDataFromAddr(tmpObj);
          ...
          if (
            ...
            this.setState(
              ...
              () => {
                ...
                // 查询银行卡列表
                // this.isDeliverAddress && this.props.paymentStoreNew.setDefaultCardDataFromAddr(this.state.deliveryAddress);
  ...
  confirmToNextPanel({ init = false } = {}) {
    ...
    if (isUserGroup && localItemRoyal.get('rc-promotionCode')) {
      // 如果是日本
      if (COUNTRY === 'jp') {
        nextConfirmPanel = paymentStoreNew?.panelStatus?.filter(
