...
class EmailForm extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.isLogin) {
      ...
      isReadyPrev && paymentStoreNew.setStsToEdit({ key: this.curKey });
    }
  }
  get isLogin() {
  ...
  handleClickConfirm = () => {
    ...
    this.props.paymentStoreNew.setDefaultCardDataFromAddr(form);
  ...
  confirmToNextPanel() {
    ...
    paymentStoreNew.setStsToCompleted({ key: this.curKey });
    paymentStoreNew.setStsToEdit({ key: nextConfirmPanel.key });
  }
  handleInputChange = (e) => {
    let { form } = this.state;
  ...
  render() {
    ...
    const { intl, paymentStoreNew } = this.props;
    const { emailPanelStatus } = paymentStoreNew;
