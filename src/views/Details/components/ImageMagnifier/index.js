...
class ImageMagnifier extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      cssStyle: {
        ...
        // 图片放大样弝
        // 此处图片宽高丝能设置为百分比，在scale的作用下，放大的坪是图片初始的宽高 ＝＝＝
        imgStyle2: {
          width: isMobile ? '230px' : '250px',
  ...
  calculationBlock(offsetX, offsetY) {
    ...
    /* 尝方块佝置 */
    // 防止鼠标移动过快导致计算失误，坪覝尝于或者大于对应值，直接设置坝移針等于最尝值或者最大值
    if (offsetX < 50) {
      offsetX = 50;
    }
  ...
  initParam() {
    ...
    cssStyle.imgStyle2.height = 'auto'; //丝能針置戝params.height,图片尺寸丝统一会坘形
  ...
  updataImg(props) {
    ...
    this.setState({
      minImg: props.minImg,
      maxImg: masterImg?.[0]?.artworkUrl || props.maxImg //有的国家丝设置主图，maxImg就会和images第一张丝一样。
    });
  }
