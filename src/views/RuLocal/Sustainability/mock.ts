const path = window.__.env.REACT_APP_EXTERNAL_ASSETS_PREFIX;

export const sustainability10013 = `${path}/img/ru-local/sustainability/10013.jpg`;
export const sustainability10018 = `${path}/img/ru-local/sustainability/10018.png`;
export const sustainability10015 = `${path}/img/ru-local/sustainability/10015.png`;
export const sustainability10016 = `${path}/img/ru-local/sustainability/10016.png`;
export const sustainability10017 = `${path}/img/ru-local/sustainability/10017.png`;
export const sustainability10014 = `${path}/img/ru-local/sustainability/10014.jpg`;
export const sustainability10025 = `${path}/img/ru-local/sustainability/10025.png`;
export const sustainability10019 = `${path}/img/ru-local/sustainability/10019.png`;
export const sustainability10021 = `${path}/img/ru-local/sustainability/10021.png`;
export const sustainability10020 = `${path}/img/ru-local/sustainability/10020.png`;
export const sustainability10022 = `${path}/img/ru-local/sustainability/10022.png`;
export const sustainability10023 = `${path}/img/ru-local/sustainability/10023.png`;
export const sustainability10007 = `${path}/img/ru-local/sustainability/10007.png`;
export const sustainability10008 = `${path}/img/ru-local/sustainability/10008.png`;
export const sustainability10009 = `${path}/img/ru-local/sustainability/10009.png`;
export const sustainability10010 = `${path}/img/ru-local/sustainability/10010.png`;
export const sustainability10011 = `${path}/img/ru-local/sustainability/10011.png`;
export const sustainability10012 = `${path}/img/ru-local/sustainability/10012.png`;
export const sustainability10024 = `${path}/img/ru-local/sustainability/10024.png`;
export const sustainability10025_p = `${path}/img/ru-local/sustainability/10025.png`;
export const sustainability10026 = `${path}/img/ru-local/sustainability/10026.png`;

export const cardList = [
  {
    strong: 'Полный перевод фабрик и офисов',
    span: ' на возобновляемую электроэнергию при одновременном снижении энергопотребления в целом.'
  },
  {
    strong: 'Закупка «экологически устойчивых» ингредиентов ',
    span: 'и сокращение выбросов углерода за счет изменения состава продуктов, включая переход на белки из возобновляемых источников и ингредиенты с низким уровнем выбросов углерода.'
  },
  {
    strong: 'Сокращение отходов и повышение цикличности',
    span: ' за счет использования возможности вторичной переработки, биоразлагаемости и использования многоразовой упаковки в соответствии с целями Mars в области устойчивого развития.'
  }
];

export const iconList = [
  {
    label: 'Пригодность для повторного использования',
    path: sustainability10018
  },
  {
    label: 'Пригодность к переработке',
    path: sustainability10015
  },
  {
    label: 'Биоразлагаемость',
    path: sustainability10016
  }
];

export const slickList = [
  {
    desc: ' «Мы стремимся предотвращать и сокращать выбросы парниковых газов на всех этапах производства. Этот вопрос мы решаем и с нашими постащиками, поскольку знаем, что 70%  выбросов углекислого газа происходит на этапе подготовки сырья».  ',
    label: 'Лоик Муто — президент ROYAL CANIN®',
    path: sustainability10019
  },
  {
    label: 'Марион Сартил — глобальный координатор по устойчивому развитию',
    path: sustainability10021,
    desc: ' «Следуя нашему научно-обоснованному подходу, мы оценили углеродный след каждой из наших формул. Наши амбиции по борьбе с изменением климата привели к одной из крупнейших трансформаций бизнеса за последние 10 лет».  '
  },
  {
    label: 'Камилла Уссельо — менеджер по стратегическому снабжению в Европе',
    path: sustainability10020,
    desc: ' «Пищевая ценность нашей продукции – наш главный приоритет. Мы всегда стараемся закупать более экологически устойчивые ингредиенты. Мы заменим рыбий жир в некоторых формулах маслом водорослей, чтобы сохранить дикую рыбу».  '
  },
  {
    label: 'Мари-Анн Урс — менеджер по научным коммуникациям в Европе',
    path: sustainability10022,
    desc: '  «Будучи ветеринарным врачом, я не только люблю животных, но и уважаю планету. В рамках программы Go Green, проводимой нашими сотрудниками, мы установили ульи на наших объектах с целью защиты черной пчелы, исчезающего вида». '
  },
  {
    label: 'Джеральдин Мера — глобальный координатор по устойчивому развитию',
    path: sustainability10023,
    desc: '  «Я обеспечиваю точность данных об устойчивом развитии, чтобынадежно отслеживать наши результаты на этом пути. Я доверяю Royal Canin и 8000 наших сотрудников, когда речь идет об улучшении нашего социального воздействия на экосистему».  '
  },
  {
    label: 'Джеффри Дэниел — менеджер по разработке рецептур',
    path: sustainability10024,
    desc: '«Разрабатывая продуктовые формулы, мы выбираем сырье, которое гарантирует оптимальную питательную ценность, оставляя минимальный углеродныйслед».  '
  },
  {
    label: 'Анн-Лор Лебё — глобальный управляющий директор по упаковке',
    path: sustainability10025_p,
    desc: '  «Пригодность для повторного использования — одна из наших главных целей при разработке новой упаковки. Я очень горжусь тем, что поддерживаю и вношу свой вклад в этот путь к устойчивому развитию». '
  },
  {
    path: sustainability10026,
    label: 'Фабрис Матье — глобальный менеджер по устойчивому развитию',
    desc: ' «Мы твердо верим, что построение экологически ответственного бизнеса основывается на продуктивном взаимодействии с партнерами и всеми заинтересованными сторонами».  '
  }
];
export const IconTabList = [
  {
    text: 'Домашние животные',
    icon: sustainability10007,
    path: sustainability10010,
    list: [
      'Наш приоритет - здоровье и благополучие кошек и собак, а также повышение осведомленности о положительной роли, которую они играют в обществе:',
      '- мы популяризируем принципы ответственного разведения и владения;',
      '- мы делимся своими знаниями, чтобы повысить осведомленность в области ухода за кошками и собаками, а также выступаем в поддержку лечебно-профилактических мероприятий;',
      '- мы распространяем информацию о положительном влиянии кошек и собак на здоровье и благополучие людей через Фонд Royal Canin.'
    ]
  },
  {
    text: 'Планета',
    icon: sustainability10008,
    path: sustainability10011,
    list: [
      'Опираясь на науку и инновации, мы стремимся сделать планету более здоровой для будущих поколений домашних животных и их владельцев:',
      '- мы стремимся уменьшить углеродный след за счет закупки "экологически устойчивых" ингредиентов;',
      '- мы работаем над сокращением количества отходов и повышением замкнутости производства;',
      '- мы рассчитываем получить сертификат об углеродной нейтральности в 2025 году.'
    ]
  },
  {
    text: 'Люди',
    icon: sustainability10009,
    path: sustainability10012,
    list: [
      'Мы твердо верим, что построение экологически ответственного бизнеса основывается на продуктивном взаимодействии с партнерами и заинтересованными сторонами:',
      '- мы работаем над снижением ущерба окружающей среде и обществу, оказываемого при транспортировке продукции;',
      '- мы поддерживаем партнеров, которые ведут деятельность ответственно и с соблюдением принципов устойчивого развития;',
      '- мы проводим обучение сотрудников, чтобы они знали нашу глобальную стратегию устойчивого развития и следовали ей.'
    ]
  }
];
