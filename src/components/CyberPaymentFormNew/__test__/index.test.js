...
jest.mock('@/components/Selection', () => {
  return ({ selectedItemChange }) => {
    ...
    return <div id='Selection' onClick={onClick}>Selection</div>;
  };
});
...
test('CyberPaymentFormNew2', async () => {
  ...
  await render(<CyberPaymentFormNew {...propsData} loginStore={{ isLogin: false }} />);

  await render(<CyberPaymentFormNew {...propsData} curPayWayInfo={{ id: false }} />);
