...
export default class Pagination extends Component {
  ...
  handlePrevOrNextPage (type) {
    ...
    const { totalPage } = this.props
    const parsedCurrentPage = isNaN(parseInt(currentPage)) ? 1 : parseInt(currentPage)
    let res
    if (type === 'prev') {
        ...
        <div className="d-flex justify-content-between align-items-center">
          ...
          </div>
          {
            currentPage >= totalPage || this.props.loading
              ? <span disabled className="rc-btn rc-pagination__direction rc-pagination__direction--prev rc-icon rc-right--xs rc-iconography" />
              : <span
