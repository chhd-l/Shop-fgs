...
const fetchDynamicConfig = async () => {
  ...
  try {
    if (baseConfig) {
      ...
      // 注释delete，原因是sonarqube error
      // if (tmpCfg?.REACT_APP_HUB_APIURL) delete tmpCfg?.REACT_APP_HUB_APIURL;
      // if (tmpCfg?.REACT_APP_BASEURL) delete tmpCfg?.REACT_APP_BASEURL;
